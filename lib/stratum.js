...
var StratumClient = function(options){
    ...
    function handleMessage(message){
        switch(message.method){
            ...
            //    handleExtraNounceSubscribe(message);
    ...
    function handleExtraNounceSubscribe(message) {
        // TODO: Apparently even trying to catch this message causes low difficulty shares to be submitted by client, more investigation needed

        // Support for mining.extranonce.subscribe is currently not written. The client sends this to let the server know
        // its supported, not handeling it is ok. The client first sends mining.subscribe, then send the extranounce command.
        // It expects a return of the extranounce value and size if supported. 
        //_this.emit('unknownStratumMethod', { method: "Currently " + message.method + " is not supported, sending not supported back to client." });
        sendJson({
            id: message.id,
            result: false,
            "error": [20, "Not supported.", null]
